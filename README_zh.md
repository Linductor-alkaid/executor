...
- **常用并发能力开箱即用**：支持优先级、延迟、软周期、批量、任务依赖、协作取消、定时句柄、FIFO 串行上下文，以及 FIFO、最新值、快照、阶段同步和 Topic 扇出。
- **失败和过载可观察**：提交拒绝、任务异常、超时、实时丢弃、队列深度、总量 admission 拒绝和通信延迟都可通过结果、状态或 callback 观察。
