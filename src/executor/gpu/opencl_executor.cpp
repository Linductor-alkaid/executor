...
bool OpenCLExecutor::copy_to_device(void* dst, const void* src, size_t size, bool async, int stream_id) {
    ...
    if (!validate_host_transfer_buffer("OpenCL copy_to_device", "source", src, size, &validation_error) ||
        !validate_device_transfer_allocation("OpenCL copy_to_device", "destination",
                                             it != memory_map_.end(),
                                             it != memory_map_.end() ? it->second.size : 0,
                                             size, &validation_error)) {
...
bool OpenCLExecutor::copy_to_host(void* dst, const void* src, size_t size, bool async, int stream_id) {
    ...
    std::string validation_error;
    if (!validate_host_transfer_buffer("OpenCL copy_to_host", "destination", dst, size, &validation_error) ||
        !validate_device_transfer_allocation("OpenCL copy_to_host", "source",
                                             it != memory_map_.end(),
                                             it != memory_map_.end() ? it->second.size : 0,
                                             size, &validation_error)) {
        set_last_error(validation_error);
...
bool OpenCLExecutor::copy_device_to_device(void* dst, const void* src, size_t size, bool async, int stream_id) {
    ...
    std::string validation_error;
    if (!validate_device_transfer_allocation("OpenCL copy_device_to_device", "source",
                                             src_it != memory_map_.end(),
                                             src_it != memory_map_.end() ? src_it->second.size : 0,
                                             size, &validation_error) ||
        !validate_device_transfer_allocation("OpenCL copy_device_to_device", "destination",
                                             dst_it != memory_map_.end(),
                                             dst_it != memory_map_.end() ? dst_it->second.size : 0,
                                             size, &validation_error)) {
        set_last_error(validation_error);
        return false;
    }
