...
本页假设你已经知道如何把函数和参数交给 Executor；如果还不清楚自由函数、成员函数、lambda 捕获和 `std::ref()` 的区别，先阅读[提交自己的函数与数据](/zh/quick-start/task-inputs-and-ownership)。
...
flowchart TD
    A{固定周期和 jitter 预算?}
    A -- 是 --> B[专用实时任务<br/>不要用普通 submit_* 代替]
    A -- 否 --> C{会重复运行?}
    C -- 是 --> D[软周期维护<br/>submit_periodic]
    C -- 否 --> E{必须等待其他任务成功?}
    E -- 是 --> F[submit_with_handle +<br/>submit_after / when_all]
    E -- 否 --> G{大量独立同类工作?}
    G -- 是 --> H[submit_batch<br/>默认保留 futures]
    G -- 否 --> I[立即 / 优先 / 延迟<br/>submit / submit_priority / submit_delayed]
```
