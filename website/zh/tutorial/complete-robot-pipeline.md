...
```text
load_map ─┐
          ├─ when_all ─> bootstrap ─> 发布配置 ─> startup phase 1
calibrate ┘                                      │
                                                 ▼
sensor thread ── SensorFrame ──> planner thread ── ControlCommand ──> control cycle
                                      │
                                      └─ SystemState ──> monitor thread
```
...
```text
停止接收外部启动/配置请求
→ 请求 sensor owner 停止，结束新帧生产
→ close sensor_frames，planner drain 已入队帧后退出
→ planner 停止产生命令，control cycle drain 或按策略丢弃剩余命令
→ 停止实时任务
→ monitor 读取最终快照后退出
→ 关闭 gate 与可关闭通道，唤醒仍在等待的线程
→ 有界等待 Executor 中的普通任务
→ shutdown Executor
→ 最后销毁 channel、mailbox、buffer 与业务对象
```
