...
2. [监控与采样](/zh/reliability/monitoring)：按开销和所需精度收集任务统计或完整生命周期快照。
3. [按症状排查运行故障](/zh/reliability/troubleshooting)：从不执行、积压、等待超时、关闭卡住、实时丢任务和 GPU 不可用获得固定检查顺序。
4. [Linux、Windows 与 Android 部署核对](/zh/reliability/platform-deployment)：检查构建、CPU、实时权限、内存锁定、Android CPU-only 降级和平台特有行为是否真实生效。
