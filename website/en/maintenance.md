...
This guide is complete when users can finish work and handle failure, not merely when every public API appears. The current baseline is `v0.4.0` plus the later `master` development snapshot; pending features are not stable-release commitments.

<div class="maintenance-hero">
  <p class="maintenance-eyebrow">CURRENT BASELINE</p>
  <p class="maintenance-version">v0.4.0 + master development snapshot</p>
