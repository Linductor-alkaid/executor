...
- **Communication Facade Observability**
  `executor::comm` components expose local `CommStats` counters for drops, overwrites, stale reads, missed phases, timeouts, depth, lag, and latency. Optional `set_event_callback()` hooks run outside internal synchronization and swallow callback exceptions, but callback configuration and invocation are diagnostic/control-plane work and are not real-time-safe. Communication events are not counted as `ExecutorFailureStatus` task failures by default.

- **Lock-Free Communication Synchronization**
  `MpscChannel<T>` and `RealtimeChannel<T>` use construction-time preallocated bounded MPSC storage with one logical consumer. `LatestMailbox<T>` and unbound SWMR `DoubleBuffer<T>` use four fixed reader-pinned snapshot slots, so non-trivial `T` is copied without a seqlock data race. Snapshot `try_publish()` is a non-waiting, system-wide lock-free operation, but contended CAS retries mean it is not per-call bounded or wait-free; `try_load()` makes at most four slot attempts. `PhaseGate` and `Sequencer` use nonblocking atomic cores. Construction rejects a platform where required synchronization atomics are not lock-free. `is_synchronization_lock_free()` describes only those internal atomics: payload operations, clocks, strings/results, callbacks, allocation outside preallocated storage, and OS scheduling remain caller/platform concerns. Timeout and guaranteed compatibility APIs spin/yield and belong on ordinary control threads. `Topic<T>`, including publish fan-out, remains mutex- and allocation-backed and is not a real-time primitive.
  Snapshot sequences have a finite 56-bit domain: exhaustion makes `try_publish()` return `false`, while retrying compatibility operations surface permanent exhaustion as `std::overflow_error` instead of spinning forever. Phase and ticket state is limited to values below `2^63`; phase/ticket waits reject larger values immediately with `InvalidArgument`, and phase-bound LET publication requires a phase below `2^63 - 1`.
