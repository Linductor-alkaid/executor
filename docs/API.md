...
| 生产者数 | 吞吐量（万/s） | 失败率 | 延迟 p99 | 与 1P 对比的效率 |
|---------|---------------|--------|----------|------------------|
| 1       | 1292.78       | 0.00%  | 362 ns   | 100.00%          |
| 2       | 460.62        | 0.00%  | 934 ns   | 17.82%           |
| 4       | 529.25        | 17.33% | 2447 ns  | 10.23%           |
| 8       | 293.02        | 80.09% | 2422 ns  | 2.83%            |
| 16      | 168.19        | 93.50% | 1877 ns  | 0.81%            |
| 32      | 175.09        | 91.56% | 1808 ns  | 0.42%            |

> 数据来源：`docs/performance/lockfree_mpsc_baseline.json`，生成时间 2026-03-16 10:50:14 CST；更新请跑 `benchmark_lockfree_mpsc_full` 并重生成 JSON。

**性能说明**：
- 单生产者性能接近理论最优（仅 3% CAS 开销）
- 2 个生产者在该基线下效率为 17.82%，失败率约为 0
- 4+ 生产者因 CAS 竞争导致失败率上升、效率显著下降
