...
```bash
./scripts/package_linux.sh --version "0.3.0"
```
...
```bash
cmake -B build -DCMAKE_PREFIX_PATH=path/to/executor-0.3.0-linux-x86_64/static
```
...
```bash
cmake -B build -DCMAKE_PREFIX_PATH=path/to/executor-0.3.0-linux-x86_64/shared
```
...
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/path/to/executor-0.3.0-linux-x86_64/shared/lib

# 方法 2: 安装到系统目录
sudo cp /path/to/executor-0.3.0-linux-x86_64/shared/lib/libexecutor.so* /usr/local/lib/
sudo ldconfig
```
...
```bash
tar -tzf dist/executor-0.3.0-linux-x86_64.tar.gz
```
...
tar -xzf dist/executor-0.3.0-linux-x86_64.tar.gz -C /tmp/package_test
