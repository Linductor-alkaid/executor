...
```powershell
.\scripts\package_windows.ps1 -Version "0.3.1"
```
...
```powershell
cmake -B build -DCMAKE_PREFIX_PATH=path\to\executor-0.3.1-windows-x64\static
```
...
```powershell
cmake -B build -DCMAKE_PREFIX_PATH=path\to\executor-0.3.1-windows-x64\shared
```
