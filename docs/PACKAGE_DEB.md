...
发布 v0.4.0 的 deb 包建议在带 CUDA Toolkit 的构建机上完成完整构建。CUDA 执行器运行时通过动态加载 `libcuda` 降级，用户机器没有 CUDA 驱动时不会阻止 CPU 路径使用。

```bash
# 在带 CUDA Toolkit 的环境下
cmake -B build -DEXECUTOR_ENABLE_CUDA=ON
sudo ./scripts/build_and_package_deb.sh --version 0.4.0 --maintainer "Your Name <your.email@example.com>"
```
...
```bash
# 打包所有类型（开发包 + 运行时包）
./scripts/package_deb.sh --version "0.4.0" --maintainer "Your Name <email@example.com>"

# 仅打包开发包
./scripts/package_deb.sh --version "0.4.0" --package-type dev --maintainer "Your Name <email@example.com>"

# 仅打包运行时包
./scripts/package_deb.sh --version "0.4.0" --package-type runtime --maintainer "Your Name <email@example.com>"
```
...
```bash
# 使用默认 all 模式，只需安装一个包
sudo dpkg -i dist/libexecutor-dev_0.4.0_amd64.deb
```

**对于仅运行应用程序的用户：**
```bash
# 只需安装运行时包
sudo dpkg -i dist/libexecutor_0.4.0_amd64.deb
```
...
sudo apt install ./dist/libexecutor-dev_0.4.0_amd64.deb
```
...
```bash
# 查看包信息
dpkg-deb -I dist/libexecutor-dev_0.4.0_amd64.deb

# 查看包内容
dpkg-deb -c dist/libexecutor-dev_0.4.0_amd64.deb

# 提取包内容（不解压）
dpkg-deb -x dist/libexecutor-dev_0.4.0_amd64.deb /tmp/extracted
```
...
- [ ] 版本号正确：`CMakeLists.txt`、README 与打包命令均为 `0.4.0`
- [ ] 维护者信息正确
- [ ] 静态库和动态库都已成功构建
- [ ] v0.4.0 发布包在带 CUDA Toolkit 的环境下完成完整构建（`EXECUTOR_ENABLE_CUDA=ON`）
...
如果用户机器没有 CUDA Toolkit，可以关闭 CUDA 构建；executor 的 CPU 功能和 OpenCL 可选路径不受影响。使用 v0.4.0 CUDA 完整包时，CUDA 后端也会在运行时检测不可用并自动降级。
...
```bash
# 先打包开发包
./scripts/package_deb.sh --package-type dev --version "0.4.0"

# 再打包运行时包
./scripts/package_deb.sh --package-type runtime --version "0.4.0"
```
